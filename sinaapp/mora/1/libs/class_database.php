 ...
 *
 *
 * %t => self::table($arg[$find])
 * %s => self::quote(is_array($arg[$find]) ? serialize($arg[$find]) : (string) $arg[$find])
 * %f => sprintf('%F', $arg[$find])
 * %F => self::quote_field($arg[ $find ]);
 * %d => dintval($arg[$find])
 * %r => $arg[$find]
 * %q => is_array($arg[$find]) ? implode(',', self::quote($arg[$find])) : self::quote($arg[$find])
 *
 *
 *
 * DB::delete($table, $condition, $limit = 0, $unbuffered = true)
 * @param $condition
 *    array('where' => 'id = %d AND username = %s', 'arg' => array(1, 'mora'))
 *    array('id' => 1, 'username' => 'mora')
 *    '`id` = 1 AND `username` = 'mora'  // 不建议用这种
 * @example
 *    DB::delete('user', array('where' => 'id = %d AND username = %s', 'arg' => array(1, 'mora')));
 *
 *
 ...
 *    获取所有行
 * @param $keyfield  指定返回的行的键 key
 *
 *
 * DB::result_first($sql, $arg = array(), $silent = false)
 *    返回结果中的第 0 行中的 第 0 列所对应的值
 *
 *
 * DB::limit($start, $limit = 0)  => ' LIMIT $start $Limit'
 *    注意返回的字符串带有 LIMIT 且前面有个空格
 *
 *
 * DB::order($field, $order = 'ASC') => '`$field` ASC'
 *    注意返回的字符串不带 ORDER BY
 *
...
class database {
	...
	public static function where($condition) {
		if (is_array($condition)) {
			if (count($condition) == 2 && isset($condition['where']) && isset($condition['arg'])) {
				return self::format($condition['where'], $condition['arg']);
			} else {
				return self::implode_field_value($condition, ' AND ');
			}
		}
		return $condition;
	}
	...
	public static function debug($output = true) {
		if (!$output) return self::$db->sqldebug;

		foreach(self::$db->sqldebug as $line) {
			echo '<p style="color:blue">' . $line[1] . "&nbsp; &nbsp;" . $line[0] . '</p>';
		}
	}
