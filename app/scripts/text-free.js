...
(function() {
  ...
  function tf(html) {
    ...
    data.striped = html;
    data.texts = result;
    data.text = result.reduce(function(last, current) { return last + current.text; }, '');
      ...
      .filter(function(node) {
        return node.nodeType === Node.ELEMENT_NODE && // 只处理 ELEMENT_NODE
                ['SCRIPT', 'STYLE', 'TEXTAREA', 'HEAD', 'HTML', 'META', 'TITLE']
                  .indexOf(node.nodeName) === -1; // 去掉特殊节点
      })
  ...
  insertCSSCode([
    ...
    '.wrap {background: white; border-radius: 6px; padding: 20px; position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);}',
    '.content {text-align: center; padding: 15px 0 30px;}',
    '.normal-text {background-color: #eee; color: #999;}',
    '.control {text-align: center;}',
    '.btn {display: inline-block;padding: 6px 12px;margin-bottom: 0;font-size: 14px;font-weight: 400;line-height: 1.42857143;text-align: center;white-space: nowrap;vertical-align: middle;cursor: pointer;-webkit-user-select: none;user-select: none;background-image: none;border: 1px solid transparent;border-radius: 4px;}',
  ...
  var tfNodes = injectClassToTfParentNode();
  ...
  document.addEventListener('click', function(e) {
    if (e.target.classList.contains(tfInjectClassName)) {
      ...
      var handler = function(e) {
        if (!e.target.classList.contains(prefix + 'btn')) { return false;}
